<?php

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class SliderModel extends CI_Model
{

    public $table = 'slider';
    public $id = 'id_slider';
    public $order = 'DESC';

    function __construct()
    {
        parent::__construct();
    }

    // get all
    function get_all()
    {
        $this->db->order_by($this->id, $this->order);
        return $this->db->get($this->table)->result();
    }

    // get data by id
    function get_by_id($id)
    {
        $this->db->where($this->id, $id);
        return $this->db->get($this->table)->row();
    }
    
    // get total rows
    function total_rows($q = NULL) {
        $this->db->like('id_slider', $q);
    $this->db->or_like('deskripsi', $q);
    $this->db->or_like('foto_slider', $q);
    $this->db->or_like('tglinput_slider', $q);
    $this->db->from($this->table);
        return $this->db->count_all_results();
    }

    // get data with limit and search
    function get_limit_data($limit, $start = 0, $q = NULL) {
        $this->db->order_by($this->id, $this->order);
        $this->db->like('id_slider', $q);
    $this->db->or_like('deskripsi', $q);
    $this->db->or_like('foto_slider', $q);
    $this->db->or_like('tglinput_slider', $q);
    $this->db->limit($limit, $start);
        return $this->db->get($this->table)->result();
    }

    // insert data
    function insert($data)
    {
        $this->db->insert($this->table, $data);
    }

    // update data
    function update($id, $data)
    {
        $this->db->where($this->id, $id);
        $this->db->update($this->table, $data);
    }

    // delete data
    function delete($id)
    {
        $this->_deleteImage($id);
        $this->db->where($this->id, $id);
        $this->db->delete($this->table);
    }
    private function _deleteImage($id){
        $row = $this->get_by_id($id);
        if ($row->foto_slider != "default.jpg") {
            $filename = explode(".", $row->foto_slider)[0];
        return array_map('unlink', glob(FCPATH."user/galeri/$filename.*"));
    }
    }

    public function detailslider($id){

        return $hsl=$this->db->query("SELECT * from slider where id_slider='$id'");
    }

}

/* End of file Foto_model.php */
/* Location: ./application/models/Foto_model.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2019-12-24 15:36:56 */
/* http://harviacode.com */